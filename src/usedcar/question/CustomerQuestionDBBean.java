...
//-------------------------------------------------------------------------------
public class CustomerQuestionDBBean { //김상헌
	private static CustomerQuestionDBBean instance = new CustomerQuestionDBBean();

	public static CustomerQuestionDBBean getInstance() {
		return instance;
	}
	...
	// 생성자
	// -------------------------------------------------------------------------------
	private CustomerQuestionDBBean() {
	}
	...
	private Connection getConnection() throws Exception {
		...
		return ds.getConnection();
			...
			// -------------------------------------------------------------------------------
			public List<MemberQuestionDataBean> getQuestionId(String user_id) throws Exception {
			...
			public int getQusetionCount(String user_id) throws Exception {
				...
				try {
					conn = getConnection();
					String sql = "select count(*) from question_list where user_id=?";
					pstmt = conn.prepareStatement(sql);
					pstmt.setString(1, user_id);
					rs = pstmt.executeQuery();
			...
			//yeonggi 191231_1639
			public void insertQuestion(MemberQuestionDataBean question){
				Connection conn = null;
				PreparedStatement pstmt = null;

				try {
					conn = getConnection();
					String sql = "INSERT INTO question_list VALUES (?,?,?,?,?,?,?)";
					pstmt = conn.prepareStatement(sql);
					pstmt.setInt(1, question.getQ_idx());
					pstmt.setInt(2, question.getCar_idx());
					pstmt.setString(3, question.getUser_id());
					pstmt.setInt(4, question.getCar_own());
					pstmt.setString(5, question.getInquiry());
					pstmt.setString(6, question.getDealer_id());
					pstmt.setTimestamp(7, question.getReg_date());
					pstmt.executeUpdate();
				} catch (Exception ex) {
					ex.printStackTrace();
				} finally {
