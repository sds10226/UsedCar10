...
<%@page import="usedcar.question.DealerQuestionDBBean"%>
...
DealerQuestionDBBean dealerQ_Process = DealerQuestionDBBean.getInstance();
CarListDBBean dealerC_Process = CarListDBBean.getInstance();//car정보들 추출하기
//------------------------------------------------------------------------------
...
List<MemberQuestionDataBean> dealerQuestion = null;
List<CarListDataBean> customerCar_List  = null;
//세션에 해당하는 아이디의 회원이 딜러에게 보낸 문의온 내역을 모두 출력한다.
...
int memberCount = 0;
memberCount = dealerQ_Process.getQusetionCount(Id);
%>    
...
<%}else{
	dealerQuestion = dealerQ_Process.getQuestionId(Id);
%>
...
<div id = "qna-infoContentTool">
 ...
 <table id="user-table">
	...
	String car_own = "";
for(int i = 0; i < dealerQuestion.size(); i++) {//문의목록에 있는 문의 하나를 꺼내서
		getCarIdx = dealerQuestion.get(i).getCar_idx();//문의한 차량의
		CarListDataBean car = dealerC_Process.getCar(getCarIdx);//정보들을 가져와서
		System.out.println(dealerQuestion);
		CustomerDataBean customer = customerProcess.getMember(dealerQuestion.get(i).getUser_id());
		int Car_own = (int)dealerQuestion.get(i).getCar_own();
		if(Car_own == 1) {
			car_own = "有";
		}else{
			car_own = "無";
...
<div id="select-container">
		...
		<span class="qReg_date"><%=sdf.format(dealerQuestion.get(i).getReg_date()) %></span>
		</button>
		
		<div class="qna-content">
			<p><%=dealerQuestion.get(i).getInquiry() %></p>
		</div>
